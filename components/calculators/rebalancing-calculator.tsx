...
interface Asset { id: string; name: string; value: number; target: number; }
...
const exampleAssets: Asset[] = [
  { id: "a", name: "미국 성장주", value: 50_000_000, target: 40 },
  { id: "b", name: "배당 ETF", value: 25_000_000, target: 30 },
  { id: "c", name: "채권", value: 15_000_000, target: 20 },
  { id: "d", name: "금", value: 10_000_000, target: 10 },
];
...
export function RebalancingCalculator() {
  const { value, setValue } = useScenarioState<State>("investment-lab-rebalance-v2", initialState);
  ...
  const add = () => setValue({ ...value, assets: [...value.assets, { id: crypto.randomUUID(), name: "새 자산", value: 0, target: 0 }] });
  ...
  const finalChartData = [
    ...result.rows.map((row) => ({ name: row.name, final: row.finalWeight * 100 })),
    ...(result.unallocatedCash > 0
      ? [{ name: "미배분 현금", final: result.finalTotal > 0 ? result.unallocatedCash / result.finalTotal * 100 : 0 }]
      : []),
  ];
  ...
  const resultNode = !result.valid ? <Card><CardContent className="p-6"><p className="font-bold text-negative">목표 비중 합계를 100%로 맞추고 자산을 2개 이상 입력해 주세요.</p></CardContent></Card> : <>
      ...
      <CardContent>
        <div className="grid gap-3 md:grid-cols-2">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            {tradePlan.unallocatedCash > 0 && <p className="mt-1 text-xs font-semibold text-slate-500">미배분 현금 {formatWon(tradePlan.unallocatedCash)}</p>}
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="mt-1 text-xs font-semibold text-slate-500">미배분 현금 {formatWon(buyOnlyPlan.unallocatedCash)}</p>
    ...
    <SecondaryResults><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="조정 후 총자산" value={formatWon(result.finalTotal)} /><MetricCard label="미배분 현금" value={formatWon(result.unallocatedCash)} tone={result.unallocatedCash > 0 ? "caution" : "default"} /><MetricCard label="평균 목표 비중 오차" value={formatPoint(result.averageGap)} tone="caution" /><MetricCard label="최대 목표 비중 오차" value={formatPoint(buyOnlySummary.maxGap)} tone={buyOnlySummary.maxGap > toleranceRate ? "caution" : "positive"} /></div></SecondaryResults>
    <AdvancedSettings title="구성 차트와 자산별 조정표" description="신규자금만 활용 기준의 구성과 자산별 배분 결과를 자세히 확인합니다.">
      <div className="space-y-4"><div className="grid gap-4 md:grid-cols-2"><ChartCard title="현재 포트폴리오"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={chartData} dataKey="current" nameKey="name" outerRadius={90} label={({ value }) => `${Number(value).toFixed(2)}%`}>{chartData.map((_, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip formatter={(number) => `${Number(number).toFixed(2)}%`} /></PieChart></ResponsiveContainer></ChartCard><ChartCard title="리밸런싱 후 포트폴리오"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={finalChartData} dataKey="final" nameKey="name" outerRadius={90} label={({ value }) => `${Number(value).toFixed(2)}%`}>{finalChartData.map((_, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip formatter={(number) => `${Number(number).toFixed(2)}%`} /></PieChart></ResponsiveContainer></ChartCard></div><Card><CardHeader><CardTitle>신규자금만 활용 기준 자산별 조정 결과</CardTitle></CardHeader><CardContent><div className="overflow-x-auto"><table className="min-w-[950px] w-full text-sm"><thead><tr className="border-b text-left"><th className="py-2">자산</th><th>현재 비중</th><th>목표 비중</th><th>목표 금액</th><th>매수</th><th>매도</th><th>예상 비중</th><th>목표 오차</th></tr></thead><tbody>{result.rows.map((row) => <tr key={row.id} className="border-b border-slate-100"><td className="py-3 font-bold">{row.name}</td><td>{formatPercent(row.currentWeight)}</td><td>{formatPercent(row.target)}</td><td>{formatWon(row.targetValue)}</td><td className="text-positive">{formatWon(row.buy)}</td><td className="text-negative">{formatWon(row.sell)}</td><td>{formatPercent(row.finalWeight)}</td><td className={Math.abs(row.finalWeight - row.target) > toleranceRate ? "font-bold text-caution" : "text-positive"}>{`${((row.finalWeight - row.target) * 100).toFixed(2)}%p`}</td></tr>)}</tbody></table></div>{result.unallocatedCash > 0 && <p className="mt-3 text-xs font-semibold text-slate-500">최소 거래단위 때문에 남은 {formatWon(result.unallocatedCash)}은 현금으로 보유하며 조정 후 총자산에 포함했습니다.</p>}</CardContent></Card></div>
    </AdvancedSettings>
  </>;

  return <CalculatorShell title="포트폴리오 리밸런싱 계산기" description="현재 자산과 목표 비중을 비교해 매수·매도 금액 또는 신규 투자금의 우선 배분안을 계산합니다." headline="어떤 자산을 더 살지 고민하기 전에 현재 비중과 목표 비중의 차이를 확인해보세요." guideHref="/guides/portfolio-rebalancing" input={input} result={resultNode} education={[{ title: "리밸런싱", body: "목표 자산배분에서 벗어난 비중을 다시 조정하는 과정입니다. 수익이 난 자산 일부를 줄이고 부족한 자산을 채울 수 있습니다." }, { title: "신규자금 방식", body: "매도를 피하고 새 투자금만으로 부족 자산을 채웁니다. 이미 과대 비중인 자산이 있으면 목표를 정확히 맞추기 어렵습니다." }, { title: "가장 많이 하는 오해", body: "최근 수익률이 좋은 자산을 계속 더 사는 방식은 위험 집중을 키울 수 있습니다." }]} assumptions={["결과에서는 매수·매도 허용 방식과 신규자금만 활용 방식을 같은 조건으로 비교합니다.", `거래금액은 ${formatWon(value.minTradeUnit)} 단위로 반올림합니다. 남은 금액은 미배분 현금으로 총자산에 포함하며, 허용 오차 ${value.tolerance}%p는 최종 목표 비중과의 차이를 비교하는 기준으로 사용합니다.`]} />;
}
