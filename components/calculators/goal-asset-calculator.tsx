...
interface State { current: number; target: number; annualReturn: number; years: number; monthly: number; inflationRate: number; mode: GoalMode; scenarioGap: number; }
const initialState: State = { current: 50_000_000, target: 300_000_000, annualReturn: 7, years: 10, monthly: 1_000_000, inflationRate: 2.5, mode: "monthly", scenarioGap: 2 };

export function GoalAssetCalculator() {
  const { value, setValue } = useScenarioState<State>("investment-lab-goal-v2", initialState);
  const result = useMemo(() => calculateGoalAsset({
    ...value,
    annualReturn: value.annualReturn / 100,
    inflationRate: value.inflationRate / 100,
    paymentTiming: "end",
    targetIsPresentValue: true,
  }), [value]);
  ...
  const scenarioData = Array.from({ length: Math.floor(months / 12) + 1 }, (_, year) => {
    ...
    return { year, 보수적: futureValue(value.current, appliedMonthly, appliedReturn - value.scenarioGap / 100, month, "end"), 기준: futureValue(value.current, appliedMonthly, appliedReturn, month, "end"), 낙관적: futureValue(value.current, appliedMonthly, appliedReturn + value.scenarioGap / 100, month, "end") };
  });
  ...
  const input = <>
    ...
    <InputCard title="핵심 목표 조건" description="목표와 현재 자산을 입력하고, 계산에 필요한 조건만 설정합니다.">
      ...
      <NumberField label="목표자산(현재가치)" value={value.target} onChange={(target) => setValue({ ...value, target })} min={0} unit="만원" inputScale={10_000} />
    ...
    <AdvancedSettings title="물가·시나리오 설정" description="목표자산은 현재가치로 입력하며, 월 투자금은 매월 말 납입으로 계산합니다.">
      <div className="space-y-5">
        <NumberField label="물가상승률" value={value.inflationRate} onChange={(inflationRate) => setValue({ ...value, inflationRate })} min={-5} max={15} unit="%" />
  ...
  const resultNode = <>
    ...
    <SecondaryResults><div className="grid gap-3 sm:grid-cols-3"><MetricCard label="예상 최종자산" value={formatWon(result.finalValue)} tone={result.finalValue >= result.adjustedTarget ? "positive" : "caution"} /><MetricCard label="현재자산 성장분" value={formatWon(futureValue(value.current, 0, appliedReturn, months, "end") - value.current)} /><MetricCard label="목표 달성률" value={formatPercent(result.finalValue / result.adjustedTarget)} tone={result.finalValue >= result.adjustedTarget ? "positive" : "caution"} /></div></SecondaryResults>
