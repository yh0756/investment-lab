...
export function LeverageCalculator() {
  ...
  const input = <>
    <Card><CardHeader><CardTitle>입력 방식</CardTitle></CardHeader><CardContent className="grid grid-cols-3 gap-2">{(["repeat","custom","preset"] as const).map((m)=><Button key={m} variant={value.mode===m?"default":"secondary"} size="sm" onClick={()=>setValue({...value,mode:m})}>{m==="repeat"?"간단 반복":m==="custom"?"직접 경로":"프리셋"}</Button>)}</CardContent></Card>
    <Card><CardHeader><CardTitle>기본 조건</CardTitle></CardHeader><CardContent className="space-y-5"><NumberField label="초기 투자금" value={value.initial} onChange={(initial)=>setValue({...value,initial})} min={0} unit="원"/><NumberField label="레버리지 배수" value={value.leverage} onChange={(leverage)=>setValue({...value,leverage})} min={1} max={5} step={0.1} unit="배" slider/><div className="grid grid-cols-3 gap-2">{[1,2,3].map((leverage)=><Button key={leverage} size="sm" variant={value.leverage===leverage?"default":"secondary"} onClick={()=>setValue({...value,leverage})}>{leverage}배</Button>)}</div><NumberField label="연간 운용보수" value={value.annualFee} onChange={(annualFee)=>setValue({...value,annualFee})} min={0} max={5} step={0.05} unit="%" slider/><div><p className="mb-2 text-sm font-semibold text-slate-700">계산 주기</p><div className="grid grid-cols-2 gap-2"><Button variant={value.periodsPerYear===252?"default":"secondary"} onClick={()=>setValue({...value,periodsPerYear:252})}>일간 재설정</Button><Button variant={value.periodsPerYear===12?"default":"secondary"} onClick={()=>setValue({...value,periodsPerYear:12})}>월간 재설정</Button></div></div><NumberField label="연간 계산 주기 수" value={value.periodsPerYear} onChange={(periodsPerYear)=>setValue({...value,periodsPerYear})} min={1} max={365} unit="회" hint="직접 조정 가능"/></CardContent></Card>
    {value.mode==="repeat"&&<Card><CardHeader><CardTitle>반복 수익률</CardTitle></CardHeader><CardContent className="space-y-5"><NumberField label="상승 구간 수익률" value={value.up} onChange={(up)=>setValue({...value,up})} min={-99} max={100} step={0.1} unit="%" slider/><NumberField label="하락 구간 수익률" value={value.down} onChange={(down)=>setValue({...value,down})} min={-99} max={100} step={0.1} unit="%" slider/><NumberField label="반복 횟수" value={value.repeats} onChange={(repeats)=>setValue({...value,repeats:Math.round(repeats)})} min={1} max={50} unit="회" slider/></CardContent></Card>}
    ...
    {value.mode==="preset"&&<Card><CardHeader><CardTitle>시나리오 프리셋</CardTitle></CardHeader><CardContent className="grid gap-2 sm:grid-cols-2">{Object.keys(presets).map((p)=><Button key={p} size="sm" variant={value.preset===p?"default":"secondary"} onClick={()=>setValue({...value,preset:p})}>{p}</Button>)}</CardContent></Card>}
  </>;
  const resultNode = <>
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3"><MetricCard label="기초지수 누적수익률" value={formatPercent(result.marketReturn)} tone={result.marketReturn>=0?"positive":"negative"}/><MetricCard label="레버리지 ETF 누적수익률" value={formatPercent(result.etfReturn)} tone={result.etfReturn>=0?"positive":"negative"}/><MetricCard label="최종 투자금" value={formatWon(result.finalEtf)} tone={result.finalEtf>=value.initial?"positive":"negative"}/><MetricCard label="단순 배수 예상수익률" value={formatPercent(result.simpleReturn)}/><MetricCard label="경로 효과" value={formatPercent(result.pathEffect)} tone={pathTone}/><MetricCard label="운용보수 누적 영향" value={formatWon(result.feeImpact)} tone="caution"/></div>
    ...
    <ChartCard title="기간별 수익률"><ResponsiveContainer width="100%" height="100%"><BarChart data={result.rows.slice(1)}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="period"/><YAxis tickFormatter={(v)=>`${Math.round(v*100)}%`}/><Tooltip formatter={(v)=>formatPercent(Number(v))}/><Legend/><Bar dataKey="marketReturn" name="기초지수" fill="#64748b"/><Bar dataKey="leveragedPeriodReturn" name="레버리지 적용" fill="#2563eb"/></BarChart></ResponsiveContainer></ChartCard>
    <DataTable title="기간별 상세 결과" rows={result.rows} columns={[{key:"period",label:"기간"},{key:"marketReturn",label:"기초지수 수익률",format:(v)=>formatPercent(Number(v)),align:"right"},{key:"marketValue",label:"기초지수 가치",format:(v)=>formatWon(Number(v),false),align:"right"},{key:"etfValue",label:"레버리지 ETF 가치",format:(v)=>formatWon(Number(v),false),align:"right"},{key:"simpleValue",label:"단순 배수 가치",format:(v)=>formatWon(Number(v),false),align:"right"}]} />
  </>;
