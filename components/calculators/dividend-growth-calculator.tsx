  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="mt-1 text-xs text-slate-500">배당률·배당성장률·주가상승률만 먼저 비교해보세요.</p>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <AdvancedSettings title="ETF 세부 조건" description="총보수와 장기 성장률 둔화 가정을 설정합니다.">
...
export function DividendGrowthCalculator() {
  ...
  const input = <>
    ...
    <AdvancedSettings title="세금 및 재투자 설정" description="기본값은 세율 15.4%, 배당 재투자 가정입니다.">
  ...
  const resultNode = <>
    <div className="grid gap-3 sm:grid-cols-3"><MetricCard label="연간 배당금 역전" value={cross(result.annualCrossover)} tone="positive" /><MetricCard label="누적 배당금 역전" value={cross(result.cumulativeCrossover)} /><MetricCard label="총자산 역전" value={cross(result.assetCrossover)} tone="positive" /></div>
    ...
    <SecondaryResults><div className="grid gap-3 sm:grid-cols-3"><MetricCard label={`${value.a.name} 최종 배당`} value={formatWon(result.finalDividend[0])} /><MetricCard label={`${value.b.name} 최종 배당`} value={formatWon(result.finalDividend[1])} /><MetricCard label="최종 총자산 차이" value={formatWon(result.finalAsset[1] - result.finalAsset[0])} tone={result.finalAsset[1] >= result.finalAsset[0] ? "positive" : "negative"} /></div></SecondaryResults>
    <AdvancedSettings title="누적 배당·총자산 상세 분석" description="장기 누적 결과와 연도별 표를 확인합니다.">
      <div className="space-y-4">
