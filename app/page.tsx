  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        <div className="mb-8"><h2 className="text-3xl font-black text-slate-950">투자 계산기 6종</h2><p className="mt-2 text-slate-600">기본 예시가 입력되어 있어 처음 방문해도 바로 결과를 확인할 수 있습니다.</p></div>
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {calculators.map(({ href, name, description, tag, icon: Icon }) => (
            <article key={href} className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-soft transition hover:-translate-y-1">
              ...
              <Link href={href} className="mt-5 inline-flex min-h-11 items-center gap-2 font-bold text-brand">계산해보기 <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></Link>
            </article>
          ))}
        </div>
      </section>
